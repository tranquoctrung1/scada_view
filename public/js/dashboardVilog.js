...
function CreateDataTable(data) {
    ...
    if (data.length > 0) {
        ...
        header += `<tr>
		<th class="font-white">Vị trí</th>
		<th class="font-white">Pin Accquy</th>
		<th class="font-white">Pin Logger</th>
		<th class="font-white">Pin Đồng hồ</th>
		<th class="font-white">Nhiệt độ</th>
		<th class="font-white">Độ ẩm</th>
		<th class="font-white">Trạng thái</th>
	   </tr>`;
        ...
        for (let item of data) {
            ...
            if (item.Status != null && item.Status != undefined) {
                if (isError == false) {
                    switch (item.Status) {
                        case 2:
                            contentError = `Gửi trễ dữ liệu. `;
                            background = '#f1c40f';
                            isError = true;
                            break;
                        case 3:
                            contentError = `Chênh lệch. `;
                            background = '#e67e22';
                            isError = true;
                            break;
                        case 4:
                            contentError = `Vượt ngưởng. `;
                            background = '#e74c3c';
                            isError = true;
                            break;
                        case 5:
                            contentError = `Accquy không sạc. `;
                            background = '#c0392b';
                            isError = true;
                            break;
                        case 6:
                            contentError = `Năng lượng dữ trữ thấp. `;
                            background = '#f39c12';
                            isError = true;
                            break;
                        case 7:
                            contentError = `Ngưng hoạt động. `;
                            background = '#d35400';
                            isError = true;
                            break;
                        case 8:
                            contentError = `Hiệu suất hoạt động giảm. `;
                            background = '#34495e';
                            isError = true;
                            break;
                        default:
                            contentError = `Bình thường. `;
                            background = '#2ecc71';
                            isError = true;
                            break;
                    }
    ...
    if (data.length > 0) {
        ...
        if (obj.LowPerformance > 0) {
            let objLowPerformance = {
                label: 'Hiệu Suất Hoạt Động Giảm',
...
function ClusterColumnChart(data, chartname, type) {
    ...
    else 
    {
        ...
        {
            lineMax = chart.series.push(new am4charts.LineSeries());
            lineMax.tooltipText = "Mức cao: {valueX}";
        ...
        lineMin = chart.series.push(new am4charts.LineSeries());
        lineMin.tooltipText = "Mức thấp: {valueX}";
    ...
    createSeries('prev2Day', '2 ngày trước');
    createSeries('prevDay', '1 ngày trước');
    createSeries('currentDay', 'ngày hiện tại');
