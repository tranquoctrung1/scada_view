    ...
    axios
        ...
        .then((res) => {
            if (res.data.length > 0) {
                ...
                LoadListChannel(inputSelectSite.value);
                LoadCurrentTimeStampBySiteId(inputSelectSite.value);
            }
        })
        ...
        .then((res) => {
            ...
            if (CheckExistsData(res.data)) {
                ...
                for (let i = 0; i < dataConvert.length; i++) {
                    if (i == 0) {
                        for (let pro of Object.getOwnPropertyNames(
                            dataConvert[0],
                        )) {
                            if (pro == 'TimeStamp') {
                    ...
                    if (data != null && data != undefined) {
                        if (data[j] != null && data[j] != undefined) {
                            if (data[j][i] != null && data[j][i] != undefined) {
                                if (
                                    data[j][i].Value != null &&
                                    data[j][i].Value != undefined
                                ) {
                                    obj[`${data[j][i].ChannelName}`] =
                                        data[j][i].Value;
                                }
                            }
                        }
